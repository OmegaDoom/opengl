               ...
               , {glm::vec3{0.f}, glm::vec3{0.f, 0.f, 10.f}, glm::vec3{0}, 0.f}}
{
    ...
    auto& framebufferDrawable0 = MakeAndAddSceneObject<FramebufferDrawable>(std::array<std::reference_wrapper<const Texture>, 2>{framebufferTexture0, framebufferTexture1}, depthAndStencilTexture);
    ...
    m_pipeline.Add(MakeAndAddSceneObject<ScreenDrawable>(), ssaoPass);
    m_pipeline.Add(MakeAndAddSceneObject<ModelNormalTransformDrawable>(glm::mat4{1.f}), ssaoPass);
    m_pipeline.Add(MakeAndAddSceneObject<SsaoDrawable>(), ssaoPass);
    auto ssaoTextures = {std::cref(framebufferTexture0), std::cref(framebufferTexture1), std::cref(depthAndStencilTexture)};
    m_pipeline.Add(MakeAndAddSceneObject<TexturesDrawable>(ssaoTextures), ssaoPass);
    m_pipeline.Add(MakeAndAddSceneObject<QuadDrawable>(2.f, 2.f, 1), ssaoPass);

    //const int screenPass = RenderPipeline::PASS2;
    ...
    //auto textures = {std::cref(framebufferTexture0), std::cref(framebufferTexture1), std::cref(depthAndStencilTexture)};
