...
QuadDrawable::QuadDrawable(Scene& scene, float width, float length, int segments)
    : Drawable(scene)
{
    ...
    for (int y = 0; y < segments + 1; ++y)
    {
        ...
        {
            vertices.emplace_back(vertex{{ -width * 0.5f + (x * width) / segments
                                          , -length * 0.5f + (y * length) / segments, 0.f }
                                          , { 0.f, 0.f, 0.f }
    ...
    m_tangent = math::CalcTangent(v1, v2, v3 
