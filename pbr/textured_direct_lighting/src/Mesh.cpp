...
Mesh::Mesh(const std::vector<vertex>& vertices, const std::vector<unsigned> indices
            , const std::vector<std::string>& diffuseTextures, const std::vector<std::string>& specularTextures, const std::vector<std::string>& normalmapTextures)
    : m_indexCount(indices.size())
{
    ...
    {
        m_diffuse.push_back(&static_cast<Texture2D&>(TextureManager::instance()[path]));
    }

    for (auto& path : specularTextures)
    {
        m_specular.push_back(&static_cast<Texture2D&>(TextureManager::instance()[path]));
    }

    for (auto& path : normalmapTextures)
    {
        m_normalmap.push_back(&static_cast<Texture2D&>(TextureManager::instance()[path]));
    }
}
...
{
    TextureMaterial textureMaterial{ 0, 1, 1.f, 2 };
