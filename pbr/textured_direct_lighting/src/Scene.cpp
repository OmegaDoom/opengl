            ...
            , 0.0019f }
{
    ...
    auto& albedo = textureManager["albedo"];
    auto& metallic = textureManager["metallic"];
    auto& roughness = textureManager["roughness"];
    auto& ao = textureManager["ao"];
    auto& normal = textureManager["normal"];
    ...
    albedo.Use(GL_TEXTURE0);
    metallic.Use(GL_TEXTURE1);
    roughness.Use(GL_TEXTURE2);
    albedo.Use(GL_TEXTURE3);
    normal.Use(GL_TEXTURE4);
    ...
    for (int y = 0; y < y_count; ++y)
    {
        for (int x = 0; x < x_count; ++x)
        {
            ...
            MakeAndAddSceneObject<PbrMaterialDrawable>(PbrMaterial{0, 1, 2, 3, 4});
