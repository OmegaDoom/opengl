            ...
            , 0.00002f }
{
    ...
    MakeAndAddSceneObject<ShaderProgramDrawable>(*this, "skinningMaterial");
    MakeAndAddSceneObject<SceneCameraDrawable>(*this);
    MakeAndAddSceneObject<SceneLightingDrawable>(*this);
    //AddSceneObject(modelLoader.Load(*this, "bob_lamp_update/bob_lamp_update_export.md5mesh"));
    AddSceneObject(modelLoader.Load(*this, "Tiny/tiny.x"));
    //AddSceneObject(modelLoader.Load(*this, "MD5/Bob.md5mesh"));
    
    m_pointLights.push_back({ glm::vec3(0.f), glm::vec3(0.1f), glm::vec3(1.f)
                            , glm::vec3(1.f), 1.f, 0.001f, 0.00001f });    
    m_pointLights.push_back({ glm::vec3(0.f), glm::vec3(0.1f), glm::vec3(1.f)
                            , glm::vec3(1.f), 1.f, 0.001f, 0.00001f });    

    m_lightInitialPositions.push_back(glm::vec4(0.f, 700.f, 0.f, 1.f));
    m_lightInitialPositions.push_back(glm::vec4(0.f, -700.f, 0.f, 1.f));

    m_lightRotationAxis.push_back(glm::vec3(1.f, 0.f, 0.f));
    m_lightRotationAxis.push_back(glm::vec3(0.f, 0.f, 1.f));
}
...
void Scene::Update()
{
    ...
    for (auto& updateble : m_updateables)
    {
        updateble.get().Update();
    }
}
