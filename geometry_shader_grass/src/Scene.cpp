            ...
            , 0.0019f }
{
    ...
    MakeAndAddSceneObject<ShaderProgramDrawable>("billboardDiscardAlpha");
    MakeAndAddSceneObject<TextureMaterialDrawable>(grassTexture, noneTexture, 1.f);

    const int GRASS_COUNT = 20000;
    ...
    for (int i = 0; i < GRASS_COUNT; ++i)
    {
        float x = dist(rng);
        float z = dist(rng);
        glm::vec3 instance{ x - PLANE_SIZE_X * 0.5f, 0.f, z - PLANE_SIZE_X * 0.5f }; 
        instances.push_back(instance);
    }

    MakeAndAddSceneObject<BillboardQuadsDrawable>(1.f, 1.f, instances);
