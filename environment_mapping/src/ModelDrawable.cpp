...
void ModelDrawable::ChangeModel(const glm::mat4& model)
{
    m_model = model;
}
...
void ModelDrawable::RenderNodes(const ModelNode& node) const
{
    for(auto meshIndex : node.MeshIndices())
    {
        ...
        ModelShaderProgram(shaderProgram).Apply(node.WorldTransform() * m_model);

        auto viewModel = scene().getCamera().View() * node.WorldTransform() * m_model;
