...
Scene::Scene()
    : m_directionalLight{ glm::vec3{0.f, 0.f, -1.f}
                        ...
                        , glm::vec3{0.9f, 0.9f, 0.9f}, glm::vec3{0.05f, 0.05f, 0.05f} }
    ...
    , m_lightView{ glm::lookAt(glm::vec3{ 0.f, 50.f, 0.f }, glm::vec3{ 0.f }, glm::vec3{ 0.f, 0.f, -1.f }) }
{
    ...
    auto& shadowMapTexture = static_cast<Texture2D&>(textureManager["depthMap"]);

    const float shadowMapProjSize = 10.f;
    m_pipeline.Add(MakeAndAddSceneObject<SceneLightingDrawable>(), RenderPipeline::PASS0 | RenderPipeline::PASS1);
    m_pipeline.Add(MakeAndAddSceneObject<ViewProjectionDrawable>(m_lightView, glm::ortho(-shadowMapProjSize, shadowMapProjSize, -shadowMapProjSize, shadowMapProjSize, 1.f, 100.f))
    ...
    m_pipeline.Add(MakeAndAddSceneObject<ShadowMapDrawable>(shadowMapTexture), RenderPipeline::PASS0);

    auto& shadowMapTextureMaterialShaderProgram = MakeAndAddSceneObject<ShaderProgramDrawable>("shadowMapTextureMaterial");

    m_pipeline.Add(MakeAndAddSceneObject<ShaderProgramDrawable>("simpleDepth"), RenderPipeline::PASS0);
    ...
    auto lightSpaceParam = [] (Scene& scene)
    {  
        const float shadowMapProjSize = 10.f;
        auto lightSpace = glm::ortho(-shadowMapProjSize, shadowMapProjSize, -shadowMapProjSize, shadowMapProjSize, 1.f, 100.f)
                                     * scene.m_lightView; 
        auto program = LightSpaceShaderProgram(*scene.m_sceneState.shaderProgram);
        program.Apply(lightSpace);
    };
    m_pipeline.Add(MakeAndAddSceneObject<CallbackDrawable<CallbackDrawableFuncT>>(lightSpaceParam), RenderPipeline::PASS1);

    auto& groundShadowMapTextureMaterialDrawable = MakeAndAddSceneObject<ShadowMapTextureMaterialDrawable>(stoneTexture, noneTexture, 1.f, shadowMapTexture);
...
void Scene::Update()
{
    ...
    //m_lightView = glm::lookAt(glm::vec3(lightPos), glm::vec3{ 0.f }, glm::vec3{ 0.f, 0.f, -1.f }); 
